...
export interface RouteStats {
  ...
  maxGrade: number;
  minGrade: number;
  smoothedMaxGrade: number;
  smoothedMinGrade: number;
...
export class GPXParser {
   ...
   * @param gpxContent - The raw GPX file content
   * @returns Success status
   */
  async parseGPX(gpxContent: string): Promise<boolean> {
    try {
  ...
  calculateSegments(): void {
    ...
    console.log(
      `Parsed ${this.segments.length} segments, total distance: ${(this.totalDistance / 1000).toFixed(2)}km`,
    );
  }

  /**
   * Generate lap boundaries at specified intervals
   * @param lapDistance - Distance of each lap in meters
   * @returns Array of lap boundary objects
   */
  generateLapBoundaries(lapDistance: number): LapBoundary[] {
   ...
   * @returns Point with lat, lon, ele
   */
  getPointAtDistance(targetDistance: number): TrackPoint {
    ...
    for (const segment of this.segments) {
      if (
        ...
        const ratio =
          (targetDistance - segment.startDistance) / segment.horizontalDistance;

        return {
          lat:
            segment.startPoint.lat +
            ratio * (segment.endPoint.lat - segment.startPoint.lat),
          lon:
            segment.startPoint.lon +
            ratio * (segment.endPoint.lon - segment.startPoint.lon),
          ele:
            segment.startPoint.ele +
            ratio * (segment.endPoint.ele - segment.startPoint.ele),
        };
      }
    }
   ...
   * @returns Average grade as decimal (0.05 = 5%)
   */
  getAverageGrade(startDistance: number, endDistance: number): number {
    ...
    return totalHorizontalDistance > 0
      ? totalElevationChange / totalHorizontalDistance
      : 0;
  }

  /**
   * Calculate average elevation between two distances
   * @param startDistance - Start distance in meters
   * @param endDistance - End distance in meters
   * @returns Average elevation in meters
   */
  getAverageElevation(startDistance: number, endDistance: number): number {
    const startPoint = this.getPointAtDistance(startDistance);
    const endPoint = this.getPointAtDistance(endDistance);
    return (startPoint.ele + endPoint.ele) / 2;
  }
   ...
   * @param minDistance - Minimum distance to look ahead (default 100m)
   * @returns Smoothed grade as decimal
   */
  getSmoothedGrade(startDistance: number, minDistance: number = 100): number {
   ...
   * @param minGradeDistance - Minimum distance for grade calculations (default 100m)
   * @returns Route statistics
   */
  getRouteStats(minGradeDistance: number = 100): RouteStats {
    ...
    const sampleInterval = Math.min(50, minGradeDistance / 4);
    const smoothedGrades: number[] = [];

    for (
      let distance = 0;
      distance <= this.totalDistance - minGradeDistance;
    ...
    return {
      ...
      netElevationChange:
        this.trackPoints[this.trackPoints.length - 1].ele -
        this.trackPoints[0].ele,
    };
  }
}
